<div x-data="{ isInfoModalOpen: @if(session('info')) true @else false @endif }">
    <div x-show="isInfoModalOpen" class="fixed inset-0 flex items-center justify-center p-5 z-99999">
        ...
        <div class="relative w-full max-w-md rounded-2xl bg-white p-6 dark:bg-gray-900 shadow-xl">
            <div class="text-center py-4">
                <div class="relative flex items-center justify-center z-1 mb-7">
                    <svg style="color:#dbeafe" width="90" height="90" viewBox="0 0 90 90" fill="none"
                    ...
                    <span class="absolute -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2">
                        <svg class="text-blue-light-500" width="38" height="38" viewBox="0 0 38 38" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                ...
                <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90 sm:text-title-sm">
                    {{session('info_title')  ?? 'معلومة!' }}
                </h4>
                <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                    {{session('info_message')  ?? 'معلومات إضافية.' }}
                </p>

                <div class="flex justify-center mt-6">
                    <button @click="isInfoModalOpen = false" type="button"
                        class="px-6 py-2 text-sm font-medium text-blue-light-500 rounded-lg bg-blue-light-500/[0.08] hover:bg-blue-light-500 hover:text-white transition-colors duration-200">
                        {{ session('info_buttonText') ?? 'حسناً' }}
                    </button>
                </div>
            </div>
        </div>
