<div x-data="{ isWarningModalOpen: @if(session('warning')) true @else false @endif }">
    <div x-show="isWarningModalOpen" class="fixed inset-0 flex items-center justify-center p-5 z-99999">
        ...
        <div class="relative w-full max-w-md rounded-2xl bg-white p-6 dark:bg-gray-900 shadow-xl">
            <div class="text-center py-4">
                ...
                <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90 sm:text-title-sm">
                    {{session('warning_title')  ?? 'تحذير!' }}
                </h4>
                <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                    {{session('warning_message')  ?? 'يرجى الانتباه.' }}
                </p>
                ...
                <div class="flex justify-center mt-6">
                    <button @click="isWarningModalOpen = false" type="button"
                        ...
                        {{ session('warning_buttonText') ?? 'حسناً' }}
                    </button>
                </div>
            </div>
        </div>
