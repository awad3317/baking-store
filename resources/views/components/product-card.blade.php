...
<div class="group flex flex-col overflow-hidden rounded-xl bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg ring-1 ring-gray-100/50">
    {{-- Product Image --}}
    ...
    <div class="flex flex-1 flex-col p-4">
        {{-- Category --}}
        @if($category)
            <span class="mb-1 text-xs font-medium text-gray-500">{{ $category }}</span>
        @endif
        
        {{-- Title --}}
        <h3 class="mb-2 text-base font-bold text-warm-brown group-hover:text-dusty-rose transition-colors">{{ $title }}</h3>
        ...
        <div class="mt-auto flex items-end justify-between">
            <div class="flex flex-col">
                ...
                @endif
                <span class="text-lg font-bold {{ $isSoldOut ? 'text-gray-400' : 'text-dusty-rose' }}">{{ $price }}</span>
            </div>
            
            @if($isSoldOut)
                <button class="flex h-9 w-9 items-center justify-center rounded-lg bg-gray-100 text-gray-400 cursor-not-allowed" disabled>
                    <span class="material-symbols-outlined text-[20px]">block</span>
                </button>
            @else
                <button class="flex h-9 w-9 items-center justify-center rounded-lg bg-stone-100 text-dusty-rose transition-all hover:bg-dusty-rose hover:text-white">
