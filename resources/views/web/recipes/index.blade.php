    ...
    <main class="flex-1" x-data="{
        ...
        <section
            ...
            <div class="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
                <div
                    class="flex items-center gap-2 overflow-x-auto md:overflow-visible no-scrollbar w-full md:w-auto pb-1 md:pb-0 z-40">
                    {{-- Reset Filter --}}
                    ...
                    <div class="relative shrink-0" x-data="{ open: false }">
                        <button @click="open = true"
                            ...
                            :class="filters.difficulty !== '' ? 'border-primary text-primary bg-primary/5' : ''">
                        ...
                        {{-- Desktop Menu (Absolute) --}}
                        <div x-show="open" @click.outside="open = false" x-transition
                            class="hidden md:flex absolute top-full right-0 mt-2 w-48 bg-white dark:bg-[#2f1f24] rounded-xl shadow-xl border border-gray-100 dark:border-[#3a2d32] z-50 flex-col py-1">
                        ...
                        <template x-teleport="body">
                            <div x-show="open" class="md:hidden relative z-[9999]">
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-300"
                                    ...
                                    class="fixed bottom-0 left-0 right-0 w-full bg-white dark:bg-[#2f1f24] rounded-t-2xl shadow-[0_-10px_40px_rgba(0,0,0,0.1)] p-6 border-t border-gray-100 dark:border-[#3a2d32]">

                                    <div class="flex items-center justify-between mb-4">
                                        <span class="font-bold text-warm-brown dark:text-white">اختر الصعوبة</span>
                                        <button @click="open = false"
                                            class="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-white/10">
                    ...
                    <div class="relative shrink-0" x-data="{ open: false }">
                        <button @click="open = true"
                            class="flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-200 bg-white hover:border-primary/30 text-dusty-dark hover:text-primary text-sm font-bold transition-all dark:bg-[#2f1f24] dark:border-[#3a2d32] dark:text-gray-300"
                            :class="filters.time !== '' ? 'border-primary text-primary bg-primary/5' : ''">
                            <span class="material-symbols-outlined text-lg">schedule</span>
                            <span x-text="filters.time ? getTimeLabel(filters.time) : 'وقت التحضير'"></span>
                            <span class="material-symbols-outlined text-sm opacity-50 transition-transform duration-200"
                                :class="open ? 'rotate-180' : ''">expand_more</span>
                        </button>
                        ...
                        <div x-show="open" @click.outside="open = false" x-transition
                            ...
                            <button @click="filters.time = ''; open = false"
                                ...
                                <span>الكل</span>
                                <span x-show="filters.time === ''" class="material-symbols-outlined text-sm">check</span>
                            </button>
                        ...
                        <template x-teleport="body">
                            <div x-show="open" class="md:hidden relative z-[9999]">
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-300"
                                    ...
                                    <div class="flex items-center justify-between mb-4">
                                        <span class="font-bold text-warm-brown dark:text-white">وقت التحضير</span>
                                        <button @click="open = false"
                                            class="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-white/10">
                                            <span class="material-symbols-outlined">close</span>
                                        </button>
                    ...
                    <div class="relative shrink-0" x-data="{ open: false }">
                        <button @click="open = true"
                            class="flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-200 bg-white hover:border-primary/30 text-dusty-dark hover:text-primary text-sm font-bold transition-all dark:bg-[#2f1f24] dark:border-[#3a2d32] dark:text-gray-300"
                            :class="filters.category !== '' ? 'border-primary text-primary bg-primary/5' : ''">
                        ...
                        {{-- Desktop Menu (Absolute) --}}
                        <div x-show="open" @click.outside="open = false" x-transition
                            class="hidden md:flex absolute top-full right-0 mt-2 w-48 bg-white dark:bg-[#2f1f24] rounded-xl shadow-xl border border-gray-100 dark:border-[#3a2d32] z-50 flex-col py-1 max-h-64 overflow-y-auto">
                            ...
                            <template x-for="cat in categories" :key="cat.slug">
                                <button @click="filters.category = cat.slug; open = false"
                                    class="flex w-full items-center justify-between px-4 py-2 text-sm text-warm-brown hover:bg-rose-50 hover:text-primary text-right transition-colors"
                                    :class="filters.category === cat.slug ? 'bg-rose-50 text-primary font-bold' : ''">
                                    <span x-text="cat.name"></span>
                                    <span x-show="filters.category === cat.slug"
                                        class="material-symbols-outlined text-sm">check</span>
                                </button>
                            </template>
                        </div>
                        ...
                        <template x-teleport="body">
                            <div x-show="open" class="md:hidden relative z-[9999]">
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-300"
                                    ...
                                    <div class="flex items-center justify-between mb-4">
                                        <span class="font-bold text-warm-brown dark:text-white">التصنيف</span>
                                        <button @click="open = false"
                                            class="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-white/10">
                                            <span class="material-symbols-outlined">close</span>
                                        </button>
